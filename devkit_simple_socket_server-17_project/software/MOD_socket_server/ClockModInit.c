...
int ParseClockRec(struct ClockRecord * record, alt_u8 * page, alt_u8 * address, alt_u8 * data)
{
	...
	*page = record->address >> 8;
	*address = record->address & 0xff;
	*data = record->data;

	return 0;	// valid record returned
};

// This was text-edited from Silicon Labs Clock Builder output file for our
// configuration of 4.6875 MHz input, 122.88 MHz output on Out3, Out7, Out8,
// and 10.0 MHz output on Out 9.  Current text is from CLK_A03.
...
struct ClockRecord ClockBody[CLOCKBODYCOUNT] =		// end-of-records is address = 0xffff data = 0xff
{
